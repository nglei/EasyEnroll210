(function($) {
    var circleMenu = {
        ...
        init: function(options) {
            ...
            $this.each(function(i) {
                var $this = $(this),
                settings = $.extend({},
                circleMenu.defaults, o),
                $menu = $('.' + settings.menu);
                $this.on('mousedown',
                function(e) {
                    if (e.which !== 3 && $(e.target).parents('.circleMenu').length < 1 && settings.click_to_close) {
                        $this.find('.circleMenu').stop(true, false).animate({
                            opacity: 0
                        },
                        {
                            duration: 100,
                ...
                function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    circleMenu.getCoords(e);
                    $('.circleMenu_close_me').stop(true, false).animate({
                        opacity: 0
                    },
                    {
                        duration: 100,
    ...
    $.fn.circleMenu = function(method, options) {
        if (circleMenu[method]) {
            return circleMenu[method].apply(this, Array.prototype.slice.call(arguments, 1))
        } else if (typeof method === 'object' || !method) {
            return circleMenu.init.apply(this, arguments)
        } else {
